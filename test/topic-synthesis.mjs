...
async function testSessionTopicUuidBridge() {
  ...
  const libraryTopicId = insertTopic(h, { name: 'canvas', updatedAt: isoMinutesAgo(60) });
  ...
  [
    'We decided that canvas topic synthesis should bridge UUID session topics.',
  ...
  const synth = new TopicSynthesizer(h.libDb, () => h.msgDb);
  const result = synth.tick(h.agentId);
  const row = fetchKnowledge(h, 'canvas');
...
async function testContentDetectorFallbackWhenSessionTopicNamesDiffer() {
  ...
  const libraryTopicId = insertTopic(h, { name: 'dashboard', updatedAt: isoMinutesAgo(80) });
  const uuidTopicId = insertSessionTopic(h, {
  ...
  [
    'We decided dashboard topic synthesis should not depend on the session topic display name.',
    'dashboard health messages can live under timestamp-derived SessionTopicMap names.',
    'The global topic detector still records dashboard from message content.',
    'Can synthesis recover when UUID topic names and library topic names diverge?',
    'Yes, dashboard content fallback should compile those source messages.',
  ].forEach((text, idx) => insertMessage(h, uuidTopicId, { text, messageIndex: idx + 1 }));
  h.libDb.prepare('UPDATE topics SET message_count = ?, updated_at = ? WHERE id = ?')
  ...
  const synth = new TopicSynthesizer(h.libDb, () => h.msgDb);
  const result = synth.tick(h.agentId);
  const row = fetchKnowledge(h, 'dashboard');
...
async function testKnowledgeVisibilityRepairOnLegacyDb() {
  ...
  const k = store.upsert('alice', 'topic-synthesis', 'after-repair', 'compiled wiki body', {
    sourceType: 'synthesizer',
