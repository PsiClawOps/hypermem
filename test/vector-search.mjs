/**
 * Vector Search (sqlite-vec) Integration Test
 *
 * Tests:
 * - sqlite-vec extension loads correctly
 * - Embedding generation works (OpenRouter or Ollama)
 ...
 *
 * Provider priority:
 * 1. OPENROUTER_API_KEY set → uses openai provider via OpenRouter (qwen/qwen3-embedding-8b)
 * 2. Ollama running locally → uses ollama provider (nomic-embed-text)
 * 3. Neither available → skip
 *
 * Requires:
 * - sqlite-vec npm package installed
 * - One of: OPENROUTER_API_KEY env var, or Ollama running locally
 */
...
async function run() {
  ...
  // ── Preflight: check embedding provider ──
  console.log('── Preflight ──');
  let providerOk = false;

  if (OPENROUTER_KEY) {
    // Verify OpenRouter is reachable with the configured model
    try {
      const resp = await fetch('https://openrouter.ai/api/v1/embeddings', {
        ...
        body: JSON.stringify({ model: 'qwen/qwen3-embedding-8b', input: ['test'] }),
        signal: AbortSignal.timeout(15000),
      });
      providerOk = resp.ok;
    } catch {}
    if (providerOk) {
      assert(true, 'OpenRouter available with qwen/qwen3-embedding-8b');
    }
  }
  ...
  if (!providerOk) {
    ...
    try {
      ...
      if (providerOk) {
        ...
        expectedDimensions = 768;
        assert(true, 'Ollama available with nomic-embed-text (fallback)');
      }
    } catch {}
  }

  if (!providerOk) {
    console.log('  ⚠️  No embedding provider available — skipping vector tests');
    console.log('  Set OPENROUTER_API_KEY or run Ollama with nomic-embed-text');
    console.log('\n═══════════════════════════════════════════════════');
    console.log('  SKIPPED (no embedding provider)');
    console.log('═══════════════════════════════════════════════════');
    process.exit(0);
  }

  console.log(`  Provider: ${embeddingConfig.provider} / ${embeddingConfig.model}`);

  // ── Test: Embedding generation ──
  console.log('\n── Embedding Generation ──');
  const embeddings = await generateEmbeddings(['Hello world', 'Goodbye world'], embeddingConfig);
  assert(embeddings.length === 2, `Generated ${embeddings.length} embeddings`);
  assert(embeddings[0].length === expectedDimensions, `Dimension: ${embeddings[0].length}`);
