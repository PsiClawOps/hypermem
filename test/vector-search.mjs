...
async function run() {
  ...
  console.log('\n── Vector Indexing ──');
  const indexResult = await hm.indexAgent('forge');
  ...
  const reindexResult = await hm.indexAgent('forge');
  ...
  // ── Test: Vector stats ──
  console.log('\n── Vector Stats ──');
  const stats = hm.getVectorStats('forge');
  ...
  const redisResults = await hm.semanticSearch('forge', 'Redis server configuration');
  ...
  const deployResults = await hm.semanticSearch('forge', 'How to deploy to production');
  assert(deployResults.length > 0, `Deploy query returned ${deployResults.length} results`);

  // Search with table filter
  const factOnlyResults = await hm.semanticSearch('forge', 'memory architecture', {
    tables: ['facts'],
    limit: 3,
  });
  assert(
  ...
  const strictResults = await hm.semanticSearch('forge', 'Redis', {
    maxDistance: 0.5,
  });
  const looseResults = await hm.semanticSearch('forge', 'Redis', {
    maxDistance: 5.0,
  });
  assert(
  ...
  hm.registerSession('agent:forge:webchat:main', 'forge', {
    channel: '#forge-main',
    channelType: 'webchat',
  });
  assert(true, 'Session registered');

  hm.recordSessionEvent('agent:forge:webchat:main', 'decision', { description: 'Chose sqlite-vec over vectorlite' });
  hm.recordSessionEvent('agent:forge:webchat:main', 'fact_extracted', { fact: 'nomic-embed-text produces 768d vectors' });
  assert(true, 'Session events recorded');

  const sessions = hm.querySessions({ agentId: 'forge', status: 'active' });
  assert(sessions.length === 1, `Active sessions for forge: ${sessions.length}`);
  ...
  hm.closeSession('agent:forge:webchat:main', 'Built vector search layer with sqlite-vec');
  const closedSessions = hm.querySessions({ agentId: 'forge', status: 'completed' });
  ...
  const events = hm.getSessionEvents('agent:forge:webchat:main');
