...
async function run() {
  ...
  try {
    hm = await HyperMem.create({
      dataDir: tmpDir,
      redis: { host: 'localhost', port: 6379, keyPrefix: 'hm-vectest:', sessionTTL: 60 },
    });
  ...
  hm.dbManager.ensureAgent('agent-alpha', { displayName: 'Agent Alpha', tier: 'council' });

  // Check sqlite-vec via getVectorDb (extension loads there, not in message DB)
  const vecDb = hm.dbManager.getVectorDb('agent-alpha');
  ...
  for (const content of factContents) {
    libDb.prepare(`INSERT INTO facts (agent_id, scope, domain, content, confidence, visibility, source_type, created_at, updated_at, decay_score)
      ...
      .run('agent-alpha', 'agent', 'infrastructure', content, 1.0, 'fleet');
  }
  ...
  libDb.prepare(`INSERT INTO knowledge (agent_id, domain, key, content, confidence, visibility, source_type, created_at, updated_at)
    ...
    .run('agent-alpha', 'operations', 'deploy-process', 'Run preflight checks, push containers, run health check, shift traffic', 0.9, 'fleet', 'conversation');

  libDb.prepare(`INSERT INTO knowledge (agent_id, domain, key, content, confidence, visibility, source_type, created_at, updated_at)
    VALUES (?, ?, ?, ?, ?, ?, ?, datetime('now'), datetime('now'))`)
    .run('agent-alpha', 'architecture', 'memory-design', 'Three-layer architecture: hot Redis compositor, warm SQLite per-agent, cold library', 0.95, 'fleet', 'conversation');
  ...
  libDb.prepare(`INSERT INTO episodes (agent_id, event_type, summary, significance, visibility, participants, created_at, decay_score)
    ...
    .run('agent-alpha', 'deployment', 'Deployed HyperMem Phase 1 with 52 passing tests', 8, 'fleet', JSON.stringify(['agent-alpha', 'testuser']));

  assert(true, `Populated ${factContents.length} facts, 2 knowledge, 1 episode`);

  // ── Test: Index all content ──
  console.log('\n── Vector Indexing ──');
  const indexResult = await hm.indexAgent('agent-alpha');
  ...
  const reindexResult = await hm.indexAgent('agent-alpha');
  ...
  // ── Test: Vector stats ──
  console.log('\n── Vector Stats ──');
  const stats = hm.getVectorStats('agent-alpha');
  ...
  const redisResults = await hm.semanticSearch('agent-alpha', 'Redis server configuration');
  ...
  const deployResults = await hm.semanticSearch('agent-alpha', 'How to deploy to production');
  assert(deployResults.length > 0, `Deploy query returned ${deployResults.length} results`);

  // Search with table filter
  const factOnlyResults = await hm.semanticSearch('agent-alpha', 'memory architecture', {
    tables: ['facts'],
    limit: 3,
  });
  assert(
  ...
  const strictResults = await hm.semanticSearch('agent-alpha', 'Redis', {
    maxDistance: 0.5,
  });
  const looseResults = await hm.semanticSearch('agent-alpha', 'Redis', {
    maxDistance: 5.0,
  });
  assert(
  ...
  const pruned = hm.pruneVectorOrphans('agent-alpha');
  assert(pruned === 1, `Pruned ${pruned} orphan(s) (expected 1)`);

  const statsAfterPrune = hm.getVectorStats('agent-alpha');
  ...
  const statsBeforeTombstone = hm.getVectorStats('agent-alpha');
  ...
  const supersededFact = libDb.prepare(`
    ...
    VALUES ('agent-alpha', 'agent', null, 'HyperMem uses Redis for hot cache (SUPERSEDED)', 1.0, 'private',
  ...
  const newFact = libDb.prepare(`
    INSERT INTO facts (agent_id, scope, domain, content, confidence, visibility,
      source_type, created_at, updated_at, decay_score)
    VALUES ('agent-alpha', 'agent', null, 'HyperMem uses Redis for hot cache (NEW)', 1.0, 'private',
      'test', datetime('now'), datetime('now'), 0.0)
  `).run();
  const newFactId = Number(newFact.lastInsertRowid);

  // Manually insert a vec_index_map entry for the superseded fact
  const tombstoneVecDb = hm.dbManager.getVectorDb('agent-alpha');
  ...
  `).run(supersededId);
  const statsWithSuperseded = hm.getVectorStats('agent-alpha');
  assert(
  ...
  const tombstoneIndexResult = await hm.indexAgent('agent-alpha');
  assert(tombstoneIndexResult.tombstoned >= 1, `Tombstoned ${tombstoneIndexResult.tombstoned} superseded entry (expected >= 1)`);

  const statsAfterTombstone = hm.getVectorStats('agent-alpha');
  ...
  hm.pruneVectorOrphans('agent-alpha');

  // ── Test: Session Registry ──
  console.log('\n── Session Registry ──');

  hm.registerSession('agent:agent-alpha:webchat:main', 'agent-alpha', {
    channel: '#agent-alpha-main',
    channelType: 'webchat',
  });
  assert(true, 'Session registered');

  hm.recordSessionEvent('agent:agent-alpha:webchat:main', 'decision', { description: 'Chose sqlite-vec over vectorlite' });
  hm.recordSessionEvent('agent:agent-alpha:webchat:main', 'fact_extracted', { fact: 'nomic-embed-text produces 768d vectors' });
  assert(true, 'Session events recorded');

  const sessions = hm.querySessions({ agentId: 'agent-alpha', status: 'active' });
  assert(sessions.length === 1, `Active sessions for agent-alpha: ${sessions.length}`);
  ...
  hm.closeSession('agent:agent-alpha:webchat:main', 'Built vector search layer with sqlite-vec');
  const closedSessions = hm.querySessions({ agentId: 'agent-alpha', status: 'completed' });
  ...
  const events = hm.getSessionEvents('agent:agent-alpha:webchat:main');
