...
const ctx = { agentId: 'agent-alpha', sessionKey: 'agent:agent-alpha:webchat:main' };

// ── Test 1: checkScope — agent scope allowed (matching agentId) ──
console.log('── checkScope: agent scope ──');
{
  const result = checkScope('agent', 'agent-alpha', null, ctx);
...
{
  const result = checkScope('agent', 'agent-beta', null, ctx);
...
{
  const result1 = checkScope(null, 'agent-alpha', null, ctx);
...
{
  const result = checkScope('session', 'agent-alpha', 'agent:agent-alpha:webchat:main', ctx);
...
{
  const result = checkScope('session', 'agent-alpha', 'agent:agent-alpha:webchat:other', ctx);
...
{
  const result = checkScope('session', 'agent-beta', 'agent:agent-alpha:webchat:main', ctx);
...
{
  const result1 = checkScope('global', 'agent-beta', 'agent:agent-beta:webchat:other', ctx);
...
{
  const result1 = checkScope('user', 'agent-alpha', null, ctx);
  assert(result1.allowed === true, 'Test 5d: user scope allowed when agentId matches');

  const result2 = checkScope('user', 'agent-beta', null, ctx);
...
{
  const result1 = checkScope('fleet', 'agent-alpha', null, ctx);
...
console.log('\n── filterByScope: mixed array ──');
{
  const items = [
    ...
    { agentId: 'agent-alpha', sessionKey: null, scope: 'agent', content: 'fact-1' },
    // Should be allowed: global scope
    { agentId: 'agent-beta', sessionKey: null, scope: 'global', content: 'fact-2' },
    // Should be allowed: null scope (defaults to agent), matching agentId
    { agentId: 'agent-alpha', sessionKey: null, scope: null, content: 'fact-3' },
    ...
    { agentId: 'agent-beta', sessionKey: null, scope: 'agent', content: 'fact-5' },
    // Should be filtered: session scope, wrong session
    { agentId: 'agent-alpha', sessionKey: 'agent:agent-alpha:webchat:other', scope: 'session', content: 'fact-6' },
    // Should be filtered: ambiguous scope
    { agentId: 'agent-alpha', sessionKey: null, scope: 'org', content: 'fact-7' },
  ];
