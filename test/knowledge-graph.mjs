...
async function run() {
  ...
  const fact1 = hm.addFact('forge', 'Redis 7.0.15 runs on localhost:6379', { domain: 'infra' });
  const fact2 = hm.addFact('forge', 'SQLite uses WAL mode for concurrent reads', { domain: 'infra' });
  const fact3 = hm.addFact('forge', 'HyperMem replaced ClawText JSONL', { domain: 'architecture' });
  const fact4 = hm.addFact('compass', 'Product launches Q2 2026', { domain: 'strategy' });
  const fact5 = hm.addFact('sentinel', 'All fleet DBs use encrypted-at-rest', { domain: 'security' });

  const k1 = hm.upsertKnowledge('forge', 'architecture', 'memory-layers',
    'L1 Redis, L2 Messages, L3 Vectors, L4 Library');
  const k2 = hm.upsertKnowledge('forge', 'architecture', 'db-split',
  ...
  const isolatedFact = hm.addFact('forge', 'Isolated fact', { domain: 'test' });
