...
function emitRerankerLog(ev) {
    ...
    if (process.env['HYPERMEM_RERANKER_LOG'] === '1') {
        console.log(`[hypermem] reranker: provider=${ev.provider ?? 'none'} candidates=${ev.candidates} status=${ev.status}`);
    }
}
// ─── FTS5 Query Building ───────────────────────────────────────
 ...
 */
async function maybeRerank(fused, query, opts) {
