...
import { hashContent, chunkMarkdown, type DocChunk } from './doc-chunker.js';
 ...
 */
export function extractWikilinks(content: string): ObsidianWikiLink[] {
 ...
 */
function readObsidianExcludedFolders(vaultPath: string): string[] {
...
function collectVaultFiles(
  ...
  for (const entry of entries) {
    ...
    if (segments.some(s => excludeFolders.has(s))) continue;
 ...
 */
export function parseObsidianNote(filePath: string, vaultRoot: string): ObsidianNote | null {
  let raw: string;
  try {
...
export function importVault(
  ...
  seenHashes: Map<string, string> = new Map()
): ObsidianImportResult {
  const {
    vaultPath,
    collection = 'obsidian/vault',
    excludeFolders: userExclude = [],
    agentId,
    importTags = true,
    importFrontmatter: _importFm = true,
  } = config;
  ...
  const allWikilinks = new Map<string, ObsidianWikiLink[]>();
  let imported = 0;
  let skipped = 0;
  let failed = 0;
