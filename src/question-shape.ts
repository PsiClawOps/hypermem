...
export const QUESTION_SHAPE_FACETS: Array<{ name: string; terms: string[] }> = [
  {
    name: 'job',
...
const FACET_TERM_SET = new Set(
  QUESTION_SHAPE_FACETS.flatMap(f => f.terms),
);
...
const TEMPORAL_SINGLE_HOP_PATTERNS: RegExp[] = [
...
const SINGLE_HOP_SUBJECT_PATTERNS: RegExp[] = [
...
export function extractQueryEntities(query: string): string[] {
  ...
  for (const m of query.matchAll(QUOTED_STRING)) {
    const val = m[1].trim();
    if (val.length >= 2) candidates.add(val.toLowerCase());
  }
...
export function extractQueryFacets(query: string): string[] {
  ...
  const matchedFacets = new Set<string>();
 ...
 */
export function extractQueryFacetTerms(query: string): string[] {
  const lower = query.toLowerCase();
...
function hasRelationWord(query: string): boolean {
  const lower = query.toLowerCase();
 ...
 */
export function questionShapeFalsePositiveScore(query: string): number {
  let fpScore = 0;
  ...
  if (wordCount < 7) fpScore += 0.15;

  // If query has no relation word at all, reduce confidence
  if (!hasRelationWord(query)) fpScore += 0.30;
