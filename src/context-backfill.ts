 ...
 */
export function backfillParentChains(
  db: DatabaseSync
): { conversationsProcessed: number; messagesUpdated: number } {
  ...
  const updateStmt = db.prepare(
    'UPDATE messages SET parent_id = ?, depth = ?, context_id = COALESCE(context_id, ?) WHERE id = ?'
  );
  ...
  for (const { conversation_id: convId } of conversations) {
    ...
    const ctxRow = db
      .prepare(
        "SELECT id FROM contexts WHERE conversation_id = ? AND status = 'active' LIMIT 1"
      )
    ...
    const messages = db
      .prepare(
        'SELECT id, message_index FROM messages WHERE conversation_id = ? AND parent_id IS NULL ORDER BY message_index ASC'
      )
      .all(convId) as Array<{ id: number; message_index: number }>;
    ...
    const lastChainedRow = db
      .prepare(
        'SELECT id, depth FROM messages WHERE conversation_id = ? AND parent_id IS NOT NULL ORDER BY message_index DESC LIMIT 1'
      )
      .get(convId) as { id: number; depth: number } | undefined;

    let prevId: number | null = lastChainedRow?.id ?? null;
    let prevDepth: number = lastChainedRow?.depth ?? -1;
