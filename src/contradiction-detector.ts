...
export interface ContradictionCandidate {
  ...
  similarityScore: number;       // 0-1 from vector/FTS
  contradictionScore: number;    // 0-1 heuristic
...
const STATE_ANTONYMS: [string, string][] = [
  ['enabled', 'disabled'],
  ['active', 'inactive'],
  ['active', 'deprecated'],
  ['running', 'stopped'],
 ...
 */
function extractNumbers(text: string): Array<{ context: string; value: number }> {
 ...
 */
function detectNegation(tokensA: string[], tokensB: string[]): boolean {
 ...
 */
function detectStateConflict(tokensA: string[], tokensB: string[]): [string, string] | null {
...
function detectNumericConflict(
  textA: string,
  textB: string,
): { contextWord: string; valueA: number; valueB: number } | null {
...
export class ContradictionDetector {
   ...
   */
  async detectOnIngest(
    agentId: string,
    newFact: { content: string; domain?: string },
  ...
  resolveContradiction(
    oldFactId: number,
    newFactId: number,
    resolution: 'supersede' | 'keep-both' | 'reject-new',
  ): void {
    switch (resolution) {
      case 'supersede':
  ...
  async autoResolve(
    ...
    if (!this.config.autoResolve) return 0;
  ...
  private async findCandidates(
    ...
    const candidates: Array<{ fact: Fact; similarity: number }> = [];
    const { maxCandidates, minSimilarity } = this.config;

    // Path 1: Vector similarity search (if VectorStore is available)
    if (this.vectorStore) {
      try {
        const vectorResults: VectorSearchResult[] = await this.vectorStore.search(
          newFact.content,
          {
            tables: ['facts'],
        ...
        for (const vr of vectorResults) {
          ...
          if (seen.has(vr.sourceId)) continue;
      ...
      if (ftsQuery) {
        try {
          ...
          for (const fact of ftsResults) {
            ...
            if (fact.supersededBy || fact.invalidAt) continue;
            seen.add(fact.id);
  ...
  private scoreContradiction(
    newContent: string,
    candidate: { fact: Fact; similarity: number },
   ...
   */
  private tokenOverlapSimilarity(textA: string, textB: string): number {
