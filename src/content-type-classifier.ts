...
export const SIGNAL_WEIGHT: Record<ContentType, number> = {
  decision:   1.0,
  spec:       0.85,
  preference: 0.8,
  skill:      0.6,
 ...
 */
export function signalWeight(content: string): number {
 ...
 */
export function isSignalBearing(content: string): boolean {
  const { type } = classifyContentType(content);
